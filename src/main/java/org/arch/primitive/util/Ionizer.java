package org.arch.primitive.util;
...
public class Ionizer {
	public static String AD_SERVERS_FILE = "ions.txt";
	...
	public Ionizer(String[] hostsSources) {
		this.hostsSources = hostsSources;
		adServers = new ArrayList<String>();

		updateIonizerList(false);
		loadListFromHostsFileFormat(getIonizerListFile());
	}

	public int getNumberOfLoadedIonizer() {
	...
	public void loadListFromHostsFileFormat(String adServersHostFile) {
		if(adServersHostFile == null || adServersHostFile.equals("")){
			adServersHostFile = getIonizerListFile();
		}
	...
	public String getIonizerListFile() {
		return Utils.getAppSettingFolder() + "/" + AD_SERVERS_FILE;
	}

	public String getLastUpdated(){
		String adServersHostFile = getIonizerListFile();
	...
	public void updateIonizerList(boolean forceUpdate) {
		log.info("Trying to update ad servers list...");

		// check if file exists
		File adServersHostFile = new File(getIonizerListFile());
	...
	public boolean downloadIonizerList() {
		String property = "java.io.tmpdir";
		String tempDir = System.getProperty(property);
		CloseableHttpClient httpClient = HttpClients.createDefault();
		boolean downloadSuccess = false; // will be false if only ALL downloads fail
		try {
			FileUtils.touch(new File(getIonizerListFile()));
		} catch (IOException e) {
			log.error("Failed to create ad hosts file " + getIonizerListFile() + ". " + e.getMessage());
		}
		// hostsSources
		...
		for (int i = 0; i < hostsSources.length; i++) {
			...
			try {
				...
				httpGet.addHeader("Referer", "https://github.com/loouislow81/");
		...
		try {
			FileUtils.writeByteArrayToFile(new File(getIonizerListFile()), FileUtils.readFileToByteArray(tempHostsFile), false);
		} catch (Exception e) {
